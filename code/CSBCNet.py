...
from Channel_Attention import Channel_Attention as CHA
from Channel_Attention import Spatial_Attention as SPA
...
class CSI_Block(nn.Module):
      def __init__(self, in_channel):
          nn.Module.__init__(self)
          self.spa = SPA(in_channel)
          self.cha = CHA(in_channel)
...
class GCAA_Block(nn.Module):
    def __init__(self, in_channel):
        super(GCAA_Block, self).__init__()
        self.linear_0 = nn.Conv2d(in_channel, 1, (1, 1))
        self.rac = Attention_Layer(in_channel)
        self.ca = Attention_Layer(in_channel)
        self.sa = Attention_Layer(in_channel)
        
    def forward(self,out,feat_piece):
        B = out.shape[0]  # Batch size
        ks = int(out.shape[2] * 0.2)
        w1x = self.linear_0(out).view(B, -1)
        tensor_w1x_reshaped = w1x.unsqueeze(-1).unsqueeze(-1)
        topk_indices = torch.topk(tensor_w1x_reshaped.view(B, -1), ks, dim=1)[1]
        expanded_indices = topk_indices.unsqueeze(1).unsqueeze(-1).expand(out.shape[0], out.shape[1], ks, 1)
        topk_out = torch.gather(out, dim=2, index=expanded_indices)
        feat_piece = self.ca(feat_piece,topk_out,topk_out)
        feat_piece = self.sa(feat_piece,feat_piece,feat_piece)
        out = self.rac(out, feat_piece, feat_piece)
        return out,feat_piece
...
class DS_Block(nn.Module):
    def __init__(self, initial=False, predict=False, out_channel=128, k_num=8, sampling_rate=0.5):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(self.in_channel, self.out_channel, (1, 1)), #4?¨°6 ?¨² 128
            nn.BatchNorm2d(self.out_channel),
            nn.ReLU(inplace=True)
        )
        self.piece_tokens = nn.Parameter(torch.randn(128, 48))        
        self.sce1 = SCE_Layer(self.k_num,self.out_channel)
        self.cfe1 =  CSI_Block(self.out_channel) 
        self.cfe2 =  CSI_Block(self.out_channel)
        self.cfe3 =  CSI_Block(self.out_channel)
        self.cfe4 =  CSI_Block(self.out_channel)
        
        self.cfe11 =  CSI_Block(self.out_channel) 
        self.cfe22 =  CSI_Block(self.out_channel)
        self.cfe33 =  CSI_Block(self.out_channel)
        self.cfe44 =  CSI_Block(self.out_channel)
        self.gcaa = GCAA_Block(self.out_channel)
          
        self.down1 = diff_pool(self.out_channel, 500)
        self.l1 = []
        for _ in range(2):
            self.l1.append(OAFilter(self.out_channel, 500))
        self.up1 = diff_unpool(self.out_channel, 500)
        ...
        self.embed_01 = nn.Sequential(
            ResNet_Block(self.out_channel*2, self.out_channel, pre=True),
            ResNet_Block(self.out_channel, self.out_channel, pre=False),
            ResNet_Block(self.out_channel, self.out_channel, pre=False),
        )
    ...
    def forward(self, x, y):
        B, _, N , _ = x.size()
        feat_piece = self.piece_tokens.unsqueeze(0).repeat(B,1,1).unsqueeze(-1)
        ...
        idx_gn1 = knn(out.squeeze(-1), k=self.k_num)
        outs = self.sce1(get_graph_feature(out, k=self.k_num, idx=idx_gn1))
        out = self.embed_01(torch.cat((outs,x_up), 1)) 

        out = self.cfe11(out)
        out = self.cfe22(out)
        out = self.cfe33(out)
        out = self.cfe44(out)      
        out,feat_piece = self.gcaa(out,feat_piece)
...
class CSBCNet(nn.Module):
    def __init__(self, config):
        super(CSBCNet, self).__init__()
