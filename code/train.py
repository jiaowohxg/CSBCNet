...
def train(model, train_loader, valid_loader, config):
    ...
    if config.resume:
        ...
        logger_train = Logger(os.path.join(config.log_path, 'log_trains.txt'), title='oan', resume=True)
        logger_valid = Logger(os.path.join(config.log_path, 'log_valid.txt'), title='oan', resume=True)
    else:
        best_acc = -1
        start_epoch = 0
        logger_train = Logger(os.path.join(config.log_path, 'log_trains.txt'), title='oan')
